...
const submitHandler = async (data) => {
  try {
      ...
      if (response.status === 200) {
          // Save token or user data in Redux or localStorage
          // Example: dispatch(login(response.data)); (for Redux)

          navigate('/test-dashboard'); // Redirect on successful login
      }
  } catch (error) {
      console.log(error);
...
<div className='w-full md:max-w-lg 2xl:max-w-3xl flex flex-col items-center justify-center gap-5 md:gap-y-10 2xl:-mt-20'>
    <span className='flex gap-1 py-1 px-3 border rounded-full text-sm md:text-base bordergray-300 text-gray-600'>
        Simplify your tasks, Amplify your success!
        </span>
<p className='flex flex-col gap-0 md:gap-4 text-4xl md:text-6xl 2x1:text-7xl font-black text-center text-amber-950'>
    <span>Organixi</span>
...
<Textbox
              ...
              type='submit'
              label='Submit'
              className='w-full h-10 bg-amber-700 text-white rounded-full'
              />
              <h1> Not a user? <a href="/Register">Register here</a></h1>
</div>
</form>
</div>
    </div>
